...
test('Dungeon scenes wire the progress and boss control scripts', async () => {
  ...
  assert.equal(roomARelicBinding.selectedPath, '', 'RoomA relic should stay image-free until final cute reward art lands.');
  assert.equal(roomBRelicBinding.selectedPath, '', 'RoomB relic should stay image-free until final cute reward art lands.');
  assert.equal(roomCRelicBinding.selectedPath, '', 'RoomC relic should stay image-free until final cute reward art lands.');
  assert.equal(roomARelicBinding.bindingStatus, 'rect_visual_placeholder');
  assert.equal(roomBRelicBinding.bindingStatus, 'rect_visual_placeholder');
  assert.equal(roomCRelicBinding.bindingStatus, 'rect_visual_placeholder');
  assert.equal(bossCoreBinding.bindingKey, 'boss_core');
  assert.equal(bossCoreBinding.selectedPath, '', 'BossEnemy-Core should stay image-free while it is a RectVisual placeholder.');
  assert.equal(bossCoreBinding.fallbackPath, '', 'BossEnemy-Core should not fall back to dark dungeon boss art.');
  assert.equal(bossCoreBinding.bindingStatus, 'rect_visual_placeholder');
  assert.equal(bossVisualController.dangerSpriteFrame ?? null, null, 'BossEnemy-Core should not bind generated boss_core sprite frames while using procedural placeholder art.');
  assert.equal(bossVisualController.vulnerableSpriteFrame ?? null, null, 'BossEnemy-Core vulnerable state should not reintroduce generated boss_core sprite frames.');
  assert.equal(bossVisualController.hurtSpriteFrame ?? null, null, 'BossEnemy-Core hurt state should not reintroduce generated boss_core sprite frames.');
  assert.equal(bossVisualController.defeatedSpriteFrame ?? null, null, 'BossEnemy-Core defeated state should not reintroduce generated boss_core sprite frames.');
  ...
  assert.ok(!getComponentTypesForNode(bossArenaItems, bossCore).has('cc.Sprite'), 'BossEnemy-Core should not own a root Sprite while placeholder-only.');
  ...
  for (const [nodeName, binding] of [
    ...
    assert.equal(binding.selectedPath, '', `${nodeName} should stay image-free while it is a RectVisual placeholder.`);
    assert.equal(binding.fallbackPath, '', `${nodeName} should not fall back to HUD or dungeon image skins.`);
    assert.equal(binding.bindingStatus, 'rect_visual_placeholder', `${nodeName} should stay marked as a non-final RectVisual placeholder.`);
